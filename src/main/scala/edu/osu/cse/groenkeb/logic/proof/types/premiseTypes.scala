...
case class Conclusion(val conclusion: ObjectRelation,
                      val rule: Rule,
                      val args: RuleArgs) extends Premise(conclusion.toSentence)
case class ProudPremise(s: Sentence) extends Premise(s) {
  def proof = CompleteProof(Conclusion(s.toRelation, NullRule(), EmptyArgs()), this :: Nil)
}
